...
while (colour == grid[counterPos] && moveOn == true) {  				//Test for run in "Forward" direction from last move position  
  ...
  if (counterPos % row != 1) 													//Test if current cell is and left end column
     { 			
  ...
  if (counterPos % row != 0)					//Test if current cell is and right end column
     { 	
     winningRun[run] = counterPos;				  
        counterPos=next;
...
function showWin()
{
	document.getElementById("winner").innerHTML = localcolour + ' is the ** WINNER **'; //Report winner to html element
	for (let i = 0; i < winRow; i++){
	   document.getElementById(winningRun[i]).style.backgroundColor = "PaleGreen"; //go through array and change colour of winning run
}
...
socket.emit('server move', val, drcol); //send the last move information to serve
}
